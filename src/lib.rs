...
pub const CURRENT_BLOCK_HEIGHT: u64 = 895000;
...
pub fn get_tx_status(tx_pool: &HashMap<String, String>, txid: &str) -> String {
    ...
    tx_pool.get(txid).cloned().unwrap_or_else(|| "not found".into())
}
...
pub fn validate_block_height(height: i64) -> (bool, String) {
    ...
    if height < 0 {
        ...
        (false, "Block height exceeds maximum unrealistic value".to_string())
    } else {
...
pub fn create_utxo(
    ...
    utxo_map.insert("txid".into(), txid.into());
    utxo_map.insert("vout".into(), vout.to_string());
